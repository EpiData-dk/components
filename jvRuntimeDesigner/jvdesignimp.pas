...
uses
  ...
  Controls, Graphics, Forms, ExtCtrls, Contnrs, JvDesignUtils, JvDesignSurface;
...
type
  ...
  protected
    { factory methods }
    function DoCreateMouseTool(ADragMode: TJvDesignDragMode): TJvDesignCustomMouseTool; virtual;
  protected
    ...
    property HandleID: TJvDesignHandleId read FHandleID write FHandleID;
    property Clicked: TControl read FClicked;
    property DragMode: TJvDesignDragMode read FDragMode;
    property KeyDownShift: TShiftState read FKeyDownShift;
  end;
  ...
  protected
    ...
    procedure ApplyDragRects; virtual;
    procedure CalcDragRects; virtual;
    procedure CalcPaintRects; virtual;
    procedure PaintDragRects; virtual;
  public
  ...
  protected
    ...
    procedure ApplyDragRect; virtual;
    procedure ApplyMouseDelta(X, Y: Integer); virtual;
    procedure CalcDragRect; override;
  public
    constructor CreateSizer(AOwner: TJvDesignSurface; AHandle: TJvDesignHandleId); virtual;
  ...
  public
    ...
    constructor Create(AMessenger: TJvDesignCustomMessenger);
    destructor Destroy; override;
...
begin
  ...
  begin
    ...
    Handles[I].ControlStyle := Handles[I].ControlStyle + [csNoDesignSelectable];
  end;
end;
...
begin
  if Selected = Container then
    ...
    P := Container.ScreenToControl(Selected.ControlToScreen(Point(0,0)));
...
const
  cCurs: array[TJvDesignHandleId] of TCursor =
   (crDrag, crSizeNWSE, crSizeNS, crSizeNE, crSizeWE, crSizeWE,
    crSizeNESW, crSizeNS, crSizeSE);
begin
...
function TJvDesignController.DoCreateMouseTool(ADragMode: TJvDesignDragMode
...
begin
  case ADragMode of
    dmNone:
      result := nil;
    dmMove:
      result := TJvDesignMover.Create(Surface);
    dmResize:
      result := TJvDesignSizer.CreateSizer(Surface, HandleID);
    dmSelect:
      result := TJvDesignBander.Create(Surface);
    dmCreate:
      result := TJvDesignBander.Create(Surface);
  end;
end;
...
function TJvDesignController.MouseDown(Button: TMouseButton; X, Y: Integer; TheMessage: TLMMouse): Boolean;
  ...
  begin
    ...
    if (csNoDesignSelectable in FClicked.ControlStyle) and
       (not (FClicked is TJvDesignHandle))
    then
      FClicked := FClicked.Parent;

    if (FClicked = Surface.Container) or
       (FClicked is TJvDesignHandle)
    then
      FClicked := nil;
  ...
  begin
    case FDragMode of
      ...
      dmMove:
        begin
          ...
          begin
            if (Surface.Selector.Count > 0) and
               (FClicked.Parent <> Surface.Selection[0].Parent)
            then
              Surface.ClearSelection;
            Surface.Selector.AddToSelection(FClicked)
          end
          else
          if (not Surface.Selector.IsSelected(FClicked)) and
             (not (csNoDesignSelectable in FClicked.ControlStyle))
            then
    ...
    FMouseTool := DoCreateMouseTool(FDragMode);
...
function TJvDesignController.MouseUp(Button: TMouseButton; X, Y: Integer; TheMessage: TLMMouse): Boolean;
  ...
  function InDragRect(C: TControl): boolean;
  var
    TL: TPoint;
    BR: TPoint;
    R: TRect;
  ...
  begin
    ...
    R := FDragRect;
    P := Surface.Container.ControlOrigin;
    OffsetRect(R, P.X, P.Y);

    TL := C.ControlOrigin;
    BR := Point(TL.X + C.Width, TL.Y + C.Height);

    if PtInRect(R, TL) and PtInRect(R, BR) then
      result := true;
  end;

  procedure SelectDragRect;
  var
    i: Integer;
    W: TWinControl;
    C: TControl;
  begin
    if FClicked = nil then exit;
    if not (FClicked is TWinControl) then exit;

    W := TWinControl(FClicked);
    for i := 0 to W.ControlCount - 1 do
    begin
      C := W.Controls[i];
      if InDragRect(C) and (not (csNoDesignSelectable in C.ControlStyle)) then
        Surface.Selector.AddToSelection(C);
    end;
  end;
...
destructor TJvDesignDesigner.Destroy;
begin
  DDC.Free;
  inherited Destroy;
end;
...
procedure TJvDesignDesigner.Notification(AnObject: TComponent;

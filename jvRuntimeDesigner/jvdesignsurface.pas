...
begin
  P := Point(AX, AY);
  C := Container.ControlAtPos(P, [capfAllowWinControls, capfRecursive]);
  if C = nil then
    C := Container;
...
begin
  Result := SelectedContainer.ScreenToClient(Container.ClientToScreen(APt));
end;
...
function TJvDesignSurface.IsDesignMessage(ASender: TControl;
  ...
  begin
    // TC: "and $FFFF" required for Windows.
